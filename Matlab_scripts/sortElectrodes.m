function elecmatrix=sortElectrodes
%   Created by:
%
...
if r==1
    while elecNr<totalnrElec
        ...
        pause(2)
        display('Ready for next electrode')
        if keydown==0 % mouse else button
            info_struct = getCursorInfo(h);
            if elecNr>0 % if second click, check whether ok
                if info_struct.Position==elecmatrix(elecNr,:)
                    disp('same position as previous electrode, try again');
                else
                    elecNr=elecNr+1;
                    elecmatrix(elecNr,:)=info_struct.Position;
                    disp(['electrode ' int2str(elecNr)]);
...
%                 plot3(info_struct.Position(1),info_struct.Position(2),info_struct.Position(3),'r.','MarkerSize',20);
            ...
            disp('press 2 to go back 1 step')
            k=input('');
            if k==2;
                elecNr=elecNr-1;
                disp(['select electrode ' int2str(elecNr+1)]);
            end
        end
    end
end
...
for filenummer=1:100
    outputnaam=strcat([outputdir '/CM_electrodes_sorted' int2str(filenummer) '.mat' ]);
